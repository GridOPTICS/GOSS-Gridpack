...
public class TransmissionElementImpl implements TransmissionElement {
    static Logger log = LoggerFactory.getLogger(TransmissionElementImpl.class);

    protected TransmissionElementImpl() {
     ...
     */
    private List<GridpackBusImpl> buses;
    ...
    protected TransmissionElementImpl(PowergridModel gridModel, Branch branch, Transformer transformer) {
        ...
        buses = new ArrayList<GridpackBusImpl>();

        log.debug("from-to bus numbers: "+fromBusNumber+ "-"+toBusNumber);
        Bus pgBusFrom = gridModel.getBus(fromBusNumber);
        if(pgBusFrom != null){
            GridpackBusImpl fromBus = GridpackBusImpl.buildFromObject(gridModel.getBus(fromBusNumber));
            buses.add(fromBus);
        }

        Bus pgBusTo = gridModel.getBus(toBusNumber);
        if(pgBusTo != null){
            GridpackBusImpl toBus = GridpackBusImpl.buildFromObject(gridModel.getBus(toBusNumber));
            buses.add(toBus);
        }
